...
//   let curDesc=
  return (
    ...
    <div className="GamePage" style={{backgroundImage: `url("images/mars1.jpg")`}}>
      ...
      <button className="change_level_button" onClick={() => handleClick()}>Change Level </button>
      ...
      <div style={{display:"flex",width:"100%",flexDirection:"column",fontFamily:"ca"}} className="Game-Level-Description">
        <div style={{display : "flex", flexWrap : "wrap",width:"100%"}} >
          <p className="game_level_text" style={{fontFamily:'Orbitron'}}>
      {curDesc}
      <br/>
          ...
          <Link to={state[level].links[0]} target="_blank" onClick={(event) => {event.preventDefault(); window.open(state[level].links[0]);}} ><button className="detail_button">click here for details</button></Link>
          :null}

          {state[level].links.length==2&&state[level].hints[1]!="<L_R_2>"?
          <Link to={state[level].links[1]} target="_blank" onClick={(event) => {event.preventDefault(); window.open(state[level].links[1]);}} ><button className="detail_button">click here for details</button></Link>
          :null}
